...
function danhsachtruongController($q, $scope, danhsachtruongService, popupFactory) {
  ...
  var rss = {
    ...
    UpdateTitle: 'Cập nhật một trường',
    UpdateButton: 'Cập nhật',
    UpdateButtonClass: 'btn-primary',

    DeleteConfirm: 'Bạn có chắc chắn muốn xóa trường này?',
    DeleteTitle: 'Xóa một trường',
    DeleteButton: 'Xóa',
  ...
  vm.create = function (id, type, refreshGridCallBack) {    
    ...
    danhsachtruongService.get(0).then(function (obj) {      
      vm.saveObj = obj;
      popupFactory.create(function () { 
        var deferred = $q.defer();
        danhsachtruongService.create(vm.saveObj).then(function () {
  ...
  vm.delete = function (row, type, refreshGridCallBack) {
    ...
    popupFactory.delete(function () { 
      var deferred = $q.defer();
      danhsachtruongService.delete(row.entity.Id).then(function () {
        deferred.resolve(true);
        if (refreshGridCallBack) {
          refreshGridCallBack();
        }
