function dantocService($q, $rootScope, $timeout, nonceProvider, dataProvider,roleFactory) {
  ...
  var _get = function (id) { 
    ...
    nonceProvider.getForVAFT().then(function (key) {
      _setKey(key);
      var dantocProvider = dataProvider.provider('dantoc');
      var obj = dantocProvider.get({id: id}, function () {
  ...
  var _create = function (obj) {
    var deferred = $q.defer();
    var dantocProvider = dataProvider.provider('dantoc', _getKey());
    dantocProvider.create({}, obj, function (res) {
  ...
  var _update = function (id, obj) {
    var deferred = $q.defer();
    var dantocProvider = dataProvider.provider('dantoc', _getKey());
    dantocProvider.update({ id: id }, obj, function (res) {
      deferred.resolve(res);
    });    
    return deferred.promise;      
  };
  ...
  var _delete = function (id) {    
    ...
    nonceProvider.getForVAFT().then(function (key) {
      _setKey(key);
      var dantocProvider = dataProvider.provider('dantoc', _getKey());
      dantocProvider.delete({ id: id }, function (res) {
        deferred.resolve(res);
      });
    });
    return deferred.promise; 
