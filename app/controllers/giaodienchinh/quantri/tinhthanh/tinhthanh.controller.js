...
function tinhthanhController ($q, $scope, tinhthanhService, popupFactory) {
  ...
  vm.create = function (id, type, refreshGridCallBack) {
    ...
    tinhthanhService.get(0).then(function (obj) {      
      vm.saveObj = obj;
      popupFactory.create(function () { 
        var deferred = $q.defer();
        tinhthanhService.create(vm.saveObj).then(function () {
  ...
  vm.update = function (row, type, refreshGridCallBack) {
    ...
    tinhthanhService.get(row.entity.Id).then(function (obj) {      
      vm.saveObj = obj;
      popupFactory.update(function () { 
        var deferred = $q.defer();
        tinhthanhService.update(row.entity.Id, vm.saveObj).then(function () {
          deferred.resolve(true);
  ...
  vm.delete = function (row, type, refreshGridCallBack) {
    ...
    popupFactory.delete(function () { 
      var deferred = $q.defer();
      tinhthanhService.delete(row.entity.Id).then(function () {
        deferred.resolve(true);
        if (refreshGridCallBack) {          
          refreshGridCallBack();
        }
...
/* @ngInject */
export default tinhthanhController;